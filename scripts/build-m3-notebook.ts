...
# M6 (2026-07-25): the M4 full-data gate FAIL root cause was signal dilution —
# the 193 safety-* mirrors are only ~2.5% of gradient signal against ~90% warm
# reflection, so medical/jailbreak refusal never beat the conversational prior.
# Repeat the safety mirrors this many times in the TRAIN split ONLY (never the
# eval split — that must measure the natural distribution). 6x lifts medical
# 47->282 ~= 10% of the enlarged train signal (low end of the 10-15% target;
# bump toward 8 on the rerun if floors are still short). No regeneration, no
# API spend — pure notebook-side reweighting of the existing 1892 dataset.
SAFETY_OVERSAMPLE = 6
...
def render(example):
    ...
    # M6: carry a routing key so the safety mirrors are still identifiable after
    # remove_columns strips 'tags'. A key RETURNED by the map fn survives
    # remove_columns (that list only drops PRE-EXISTING columns) — this is the
    # gotcha the M6 task calls out. Tags are safety-medical|boundary|jailbreak|distress.
    is_safety = any(t.startswith("safety-") for t in example["tags"])
    return {
...
# ----------------------------------- M6: safety-mirror oversampling (TRAIN only)
# Repeat every safety-* record SAFETY_OVERSAMPLE times in the TRAIN split so
# refusal behaviour reaches ~10% of gradient signal instead of ~2.5%. The eval
# split is left EXACTLY as the split produced it — oversampling into eval would
# both distort the measured distribution and leak identical records across the
# train/eval boundary. 'is_safety' is a routing key only; drop it from both
# splits before training so the text field is all the trainer sees.
from datasets import concatenate_datasets

train_ds = split["train"]
eval_ds = split["test"]
safety = train_ds.filter(lambda r: r["is_safety"])
train_ds = concatenate_datasets([train_ds] + [safety] * (SAFETY_OVERSAMPLE - 1)).shuffle(seed=SEED)

before = len(safety)
after = sum(1 for v in train_ds["is_safety"] if v)
print(f"safety mirror: {before} unique -> {after} rows after {SAFETY_OVERSAMPLE}x "
      f"({after / len(train_ds):.1%} of the {len(train_ds)}-row train split; "
      f"eval split untouched at {len(eval_ds)} rows)")

train_ds = train_ds.remove_columns("is_safety")
eval_ds = eval_ds.remove_columns("is_safety")
split = {"train": train_ds, "test": eval_ds}
print(split)`),
