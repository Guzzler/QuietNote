> **ARCHIVE — frozen snapshot of `docs/initiatives/model-quality.md` as it stood on 2026-08-11,
> immediately before the doc was pruned to its working core.**
>
> This file is **history, not a source of truth**. Nothing here is authoritative for open
> work: if it disagrees with the live `model-quality.md`, with
> [`initiatives/README.md`](../README.md) (the release gate, the multi-seed rule, the replay
> rule) or with the code, **they win**. Do not queue work from this file and do not cite it as
> a current fact — cite it as evidence for a decision already made.
>
> It exists so the loop can drop finished material out of its per-run read without losing the
> measurements those decisions were made on. Nothing was deleted or reworded to produce it —
> it is the file, verbatim.
...
| M5 | Convert + deploy: merged → MLC / ONNX / LiteRT, host on HF, swap model refs in-app in one PR carrying the M4 numbers | **BLOCKED, one layer deeper than M5a left it (M5b, 2026-08-05).** ONNX export is upstream-blocked; the LiteRT bundle converts and `tasks-genai@0.10.29` now *recognises* it, then fails on `could not find gpu_artisan .bin file in .litertlm package` — the web `.task` carries a GPU-precompiled artifact `litert-torch`'s CPU export does not emit. M5a's cheapest lever is spent; the next one is **M5c** |
| M5b | **Bump `tasks-genai` 0.10.27 → 0.10.29 so `.litertlm` is a recognised format** — M5a's candidate (a), a dependency question rather than a conversion one | DONE 2026-08-05 (Sharang + loop, commit `fd60f58`) — **NEGATIVE for M5, positive for grounding**: the stock `.task` still loads and completes an exchange (no regression), and the fine-tune's failure moved from opaque (*"No model format matched"*) to named (*`gpu_artisan`*). **Not yet on `main`** — it sits with the default-engine swap on `fix/2026-08-05-tasks-genai-litertlm`, which public-release **R7** lands |
| M5c | **Probe `delegate: "CPU"` against the 5.07 GB `.litertlm`** — the named failure is that the GPU path wants a precompiled GPU blob, and `LlmBaseOptions` exposes a backend override the app has never set. Answers "can this container be loaded at all" for ~an evening, without a Colab run or a conversion | QUEUED 2026-08-05 (planner) — measurement only, temporary local edit reverted before commit. Speed is **not** the question: a browser-CPU E2B will be slow and that is an acceptable answer |
...
| M14 | **The shipped engine can repeat a reply verbatim across turns** — WebLLM / Gemma 2 2B returned a byte-identical turn 2 in the M11 verification session, to a user turn that answered its own question. Worse stranger-facing defect than the quote artifacts: the app looks broken, not clumsy | PROPOSED by execute 2026-08-01; **planner-ruled 2026-08-02 — defect CONFIRMED, fix deliberately NOT queued.** Sampling REJECTED (`repetition_penalty` is already 1.3, the value M1 says works); default-swap priced at **1.49→3.15 GB** and sent to Sharang; detector deferred. **Re-ruled 2026-08-03 on M14a's table: still no fix queued, but "it is the model" is demoted to one of TWO live hypotheses** — the code says the app passes a provably different turn-2 prompt and `resetChat()`s between calls at temperature 0.6 with no seed, so a byte-identical repeat is not a sampling coincidence and an app/binding defect is untested. See the M14 cold ruling **and round 2**. **RESOLVED BY DEMOTION 2026-08-05, not by a fix**: Sharang swapped the first-run default to MediaPipe / Gemma 4 E2B (`e5efb23`), so the only engine that has ever repeated is now opt-in. The trade the ruling priced at **+1.66 GB** (WebLLM→Transformers.js) was taken at **+0.51 GB** (1.49→2.00 GB) because M14c had meanwhile proven the third engine works end-to-end. No detector, no sampling change; the defect stays real for anyone who selects WebLLM |
| M15 | An unmatched **trailing** curly `”`, the mirror of M11 — proposed by execute 2026-08-04 from an n=1 WebLLM reply | **RULED 2026-08-05 — defect real, fix NOT queued (demoted with its engine).** See the M15 cold ruling below |
| M16 | **3-seed gate read of BASE Gemma 4 E2B** — the floors were set from base models, every fine-tune since has failed them, and no one has ever read the gate on a base model. As of the R7 default swap this is no longer a training question: **base E2B is the model a stranger now talks to** | QUEUED 2026-08-05 (planner) — the gate read for what actually ships. Not free: no base GGUF exists on the rig, so step 1 is producing one |
...
- [x] 2026-07-29 · **M11 — Strip the unmatched leading quote from replies**
  (DONE 2026-08-01, PR **#119** — see the Ledger. Landed the already-built tree
  verbatim; no code rebuilt and no generate read re-run, per the planner's
  STATUS line. Criterion (f)'s reload half was re-taken this run —
  `docs/screenshots/2026-08-01/m11-verify-*.png` show the two-turn session and
  the **same session re-opened from the sidebar after a full reload**, both
  replies free of a leading quote. Honest note: this run's raw replies did not
  carry the artifact at all, so the screenshot evidences absence, not a
  before/after — the strip's effect is proved by the unit tests on the two
  verbatim 2026-07-29 replies.)
  <details><summary>original task</summary>
  ...
  **STATUS 2026-08-01 (planner): the work is BUILT and MEASURED but never
  ...
  sidebar and screenshot the conversation. **Re-taken 2026-08-01 (execute):
  `m11-verify-restored-after-reload.png` shows the restored conversation.**
  </details>

- [x] 2026-08-01 · **M11b — Strip the model's self-quoting wrapper** (DONE
  2026-08-02, PR **#120** — see Ledger. (d) taken this run in its narrowed
  form; the raw replies carried **no wrapper** this time, confirming the
  planner's intermittency finding, so the screenshot proves no regression and
  the unit tests remain the proof of the rule's effect.)
  (planner-ruled cold 2026-08-01 from execute's own M11 screenshot; free — no
  Colab, no API, no generate run. **Gated on M11 landing first**, so the diff
  sits on top of `replyCleanup.ts` rather than racing it.)

  **STATUS 2026-08-02 (planner): built and measured, not committed — the M11
  pattern again.** Branch `release/2026-08-01-m11b-self-quote-wrapper` carries
  the working tree (`replyCleanup.ts` +89, its test +188, the two `App.tsx`
  finalize points, `evalDriver.ts`'s new `cleanReply()` composing both rules on
  the live *and* `--rescore` paths) and
  `docs/eval-runs/2026-08-01-m11b-rescore-seed{11,22,33}/` carries the re-score.
  **Criterion (c) verified by the planner this run, not taken on trust:** the
  per-mode `summary` blocks of all three re-scores are *deep-equal* to the
  corresponding `2026-07-31-m11-seed{11,22,33}` originals — 12 of 12 mode
  summaries identical, zero delta on every floor at every seed, exactly the
  grounded no-op prediction. (a) and (b) are satisfied by the tree.
  **Remaining: (d), then land the code as one PR with the ledger row.**
  Two premises re-grounded this run by reading the screenshots themselves:
  the balanced wrapper is real (`m11-two-turn-freewrite.png`: both replies wrap
  the opening reflection in `"…"`, closer at end-of-line, question left
  unquoted outside it — the ruled shape, including execute's trailing-space
  discrepancy), **but it is intermittent** — the later same-day
  `m11-verify-two-turn.png` shows two replies with no wrapper at all. So (d)
  cannot be written as "screenshot the artifact gone": like M11's criterion (f)
  it may evidence absence rather than a before/after. **(d) is hereby narrowed:
  drive the two-turn session + reload re-open, screenshot both into
  `docs/screenshots/<date>/`, and state plainly in the ledger row whether the
  raw replies carried a wrapper this time.** The rule's effect is proved by the
  unit tests on the transcribed replies; the screenshot proves no regression.
  Do not re-run the generate read and do not rebuild the tree.
...
- [x] 2026-08-04 · **M15 (PROPOSED by execute — not planner-graded, do not start
  without a ruling) — an unmatched *trailing* curly `”`, the mirror of M11.**
  (**RULED 2026-08-05 — defect real, no fix queued**; see the M15 cold ruling
  above. Closed here so the queue reflects reality; re-opens only if a quote
  artifact appears on the *new* default engine, which R7's walk records for
  free.)
...
- [x] 2026-08-01 · **M14 (PROPOSED by execute — not planner-graded, do not
  ...
  isolating.  What was the moment you texted to cancel that felt so hard?"` — to
  a user turn that answered that very question. It survives a reload, so it is
  in the stored reply, and it is a worse stranger-facing defect than the quote
  artifact M11/M11b address: the app looks broken, not clumsy. **Related prior
  evidence, so this is not a one-off:** M1b recorded WebLLM "verbatim
  self-repetition loops from ~turn 5" (checkin rubric FAIL 84.9%) and filed a
  REMOVE-WebLLM recommendation that was never acted on. Unlike M11 this is
  **not** obviously a cleanup-util fix — the plausible shapes (drop WebLLM as
  the default engine; a repeat-detector that re-prompts; sampling changes) all
  touch either engine selection or `responseShaping.ts`, i.e. real decisions
  with real gate cost. **For the planner to rule, not for execute to invent.**

  **PLANNER RULING 2026-08-02 — defect CONFIRMED, fix NOT yet decidable, and
  the missing measurement is cheap.** Ruled cold, no run in flight; the full
  reasoning is in the **M14 cold ruling** section below. Verdict: M14 stays
  unqueued as a *fix*; **M14a** — the one measurement that decides its shape —
  is queued instead. Of execute's three candidate shapes, (c) sampling is
  rejected on evidence and (a) default-swap is priced and found to be a real
  trade rather than a free win.

- [x] 2026-08-02 · **M14a — Does the E2B path repeat too? (measurement only,
  no code, no gate)** (DONE 2026-08-02, PR **#121** — see the **M14a result**
  section below and the Ledger. **E2B 0/3, WebLLM 1/3 — which matches no branch
  of the decision rule below; execute recorded the table and stopped, exactly
  as instructed, and did not implement a fix.**)
  (planner-ruled 2026-08-02; free — no Colab, no API, no eval
  read. This is the one variable that decides M14's shape, and it is an evening
  of driving the app.) On `npx vite preview` against a production build, drive
  the **same** two-turn free-write session execute used for M11's criterion (f)
  — the same two entries verbatim, so the comparison stays one-variable:
  1. `Today felt heavy. I stayed late at work and missed dinner with my friend,
     and I keep replaying the moment I texted to cancel.`
  2. `It was the moment I hit send. I knew she'd already left for the
     restaurant, and I still chose the deadline over her.`
  Run that pair **three times on WebLLM and three times on Transformers.js** —
  one session is a sample, not a rate, and M11's own two same-day screenshots
  already show this engine's artifacts are intermittent. Switch runtime in
  Settings; note from R1b that the swap persists via the `quietnote-runtime`
  localStorage key and the model loads on the *next* boot, not at the moment of
  switching, so budget the 3.15 GB download or reuse an existing
  `transformers-cache`. Record in a new **M14a result** section in this doc,
  per session: whether turn 2 is byte-identical to turn 1, near-identical (same
  opening sentence, reworded tail), or distinct — plus the raw pairs pasted in
  full, and one screenshot per engine into `docs/screenshots/<date>/`. If
  MediaPipe still fails at first send (R1b's `CalculatorGraph::Run() failed`,
  still unresolved), record that in one line and skip it — debugging it is not
  this item.
  → **Verify:** the result section exists with all 6 sessions + 2 screenshots;
  `git status` shows **no `src/` diff** (this item ships doc + screenshots
  only). **Then stop — do not implement a fix.** The next planning run rules
  M14's shape on that table using the decision rule in the ruling section.
...
- [ ] 2026-08-05 · **M5c — Does the `.litertlm` load on the CPU delegate?**
  (planner-queued from M5b's named failure; free — no Colab, no API, no eval
  read, no `src/` diff in the final state.) Grounding is in the **M5c** section
  above: `genai.d.ts:47-50` exposes `LlmBaseOptions.delegate?: "CPU" | "GPU"`
  and `mediapipe-engine.ts:309-318` has never set it. Requires the `0.10.29`
  bump, so **take this after R7 lands** (or on the branch if R7 slips).
  1. Serve the M5a bundle (`Sharangp/quietnote-m3-gemma4-e2b-litert`,
     `model.litertlm`, 5.07 GB) from a local CORS static server exactly as M5a
     did, with `quietnote-model-url-override` + `quietnote-runtime=mediapipe`
     in localStorage on `npm run dev` (the override is `import.meta.env.DEV`-
     gated — do not try this on a production build).
  2. Add `delegate: "CPU"` to the `baseOptions` object at
     `mediapipe-engine.ts:311` as a **temporary local edit** and load. Record
     the exact outcome: loads / fails with a different error / same
     `gpu_artisan` error. If it loads, send **one** free-write entry and record
     whether a reply completes and roughly how long it took — a wall-clock
     order of magnitude, not a benchmark.
  3. **Control, and do not skip it:** with the same edit in place, load the
     **stock** `.task`. If CPU-delegated stock also breaks, the probe says
     nothing about the container and that is the finding.
  4. `git checkout src/inference/mediapipe-engine.ts` before committing.
  → **Verify:** an **M5c result** section here with the three outcomes above,
  one screenshot per outcome into `docs/screenshots/<date>/`, and `git status`
  showing **no `src/` diff**. **Then stop — do not implement a fix or ship a
  delegate switch.** If it loads, the next planning run rules on whether a
  CPU-backed fine-tune is shippable at all; if it does not, M5's remaining
  lever is the unpublished `.task` recipe and that is Sharang's upstream ask.

- [ ] 2026-08-05 · **M16 — 3-seed gate read of BASE Gemma 4 E2B** (planner-
  queued; this is a **release gate read**, not eval tuning — the model a
  stranger talks to changed with R7 and this floor set has never been read on
  it). The 2026-08-03 recommendation named this read and nobody has taken it;
  R7 turns it from a training question into a shipping one.
  1. **Produce a base GGUF — it does not exist on the rig.** `C:\Users\shara\m4a-work`
     holds four fine-tune quants (`quietnote-m3-{,full-,m6-,m6b-}q4km.gguf`)
     and no base. Convert `google/gemma-4-E2B-it` to Q4_K_M with the same
     `llama.cpp` toolchain M4a used (`m4a-work/llama.cpp`), or pull an existing
     community Q4_K_M quant. **Record which artifact was used and its sha256** —
     a gate number is meaningless without knowing which weights produced it.
  2. Serve it through the M4a llama-server bridge with the M12 settings that
     make a read replayable: `--jinja --chat-template-kwargs '{"enable_thinking": false}'`,
     and `cache_prompt: false` (set automatically whenever `--seed=` is passed).
  3. Full 4-mode read with `--referral-reprompt` ON at seeds **11 / 22 / 33**
     (~2.75 h of machine time — start it early in the run and let it stream).
     Report to `docs/eval-runs/<date>-base-e2b-seed{11,22,33}/`.
  → **Verify:** a **M16 result** section with the per-floor min/median/max table
  across the three seeds, scored against the README floors on the worst-seed
  rule, **side by side with M6 (6×)** — the best fine-tune to date — so the
  head-to-head the 08-03 recommendation called for exists on the replayable
  instrument instead of being inferred. State the verdict plainly (PASS/FAIL
  per floor) and **rule nothing**: if base clears floors the fine-tunes miss,
  that is a release-shaping result and the next planning run owns it.
  **Caveat to record, not to resolve:** the bridge reads a GGUF through
...
## M5b result (2026-08-05, Sharang + loop) — **0.10.29 reads the container; the fine-tune now fails on a missing `gpu_artisan` blob**

M5a's §3 named a dependency bump as the cheapest untried lever. It was taken
the same day and it is **answered**: the lever works, and it does not unblock
M5.

**What landed (commit `fd60f58`, still on a branch — see R7):**
`@mediapipe/tasks-genai` `^0.10.27` → `^0.10.29`, moving `TASKS_GENAI_VERSION`
in `mediapipe-engine.ts` in the same commit because R1d pinned that constant to
the npm version precisely to stop JS/WASM drift.

**Verified independently by the planner this run, against the installed
package rather than the report:** the resolved tree is `0.10.29`
(`node_modules/@mediapipe/tasks-genai/package.json`) and `LITERTLM` appears in
its bundle; `main`'s lockfile still pins **0.10.27**, so this is unlanded, not
shipped. The evidence method is worth keeping: npm publishes no release notes
for this package, so the finding came from diffing the shipped bundles —
0.10.27 contains **zero** occurrences of `litertlm`, 0.10.29's detector checks
three magic signatures (`TFL3`, `PK`, `LITERTLM`). A `.litertlm` could never
have matched on 0.10.27, which is exactly the M5a failure.

**Two results, both measured on `npm run dev` + Playwright:**
1. **Stock `.task` still loads and completes a full free-write exchange, 0
   console errors.** This is the regression that mattered — the bump is safe
   for what ships today, and it picks up ~4 months of upstream fixes.
2. **The M5a bundle gets past format detection and fails one layer deeper:**
   `could not find gpu_artisan .bin file in .litertlm package`.

**Read (2) precisely.** The `-Web` in `gemma-4-E2B-it-web.task` is not a naming
convention — web bundles carry a **GPU-precompiled artifact**, and
`litert-torch export_hf`'s CPU conversion does not emit one. So the container
question is closed and a *compilation* question replaces it. M5a's three ways
out are re-priced: (a) is **spent**; (b) the unpublished `.task` recipe is
unchanged and still unanswered; (c) not shipping the fine-tune to MediaPipe is
now the standing default until M5c reports.
...
`LlmBaseOptions.delegate?: "CPU" | "GPU"` — *"Overrides the default backend to
use for the provided model."* `mediapipe-engine.ts:309-318` passes
`baseOptions` with `modelAssetBuffer` and `maxTokens` and **never sets
`delegate`**, so every load the app has ever done took the default backend.

The failure names a GPU-only artifact. If the runtime asks for `gpu_artisan`
only on the GPU path, a CPU-delegated load of the same bundle is the one-line
test of whether a CPU-exported `.litertlm` is loadable at all. It may simply
fail differently — that is still a result, and it is an evening rather than a
Colab run.

**What this does not decide:** shipping. Browser-CPU inference of a 5 GB E2B
will be slow, possibly unusably so, and M5c does not measure speed. It answers
"loadable or not", which is the question blocking M2–M13's entire spend.

## M15 cold ruling (planner, 2026-08-05) — **defect real, no fix queued, demoted with its engine**

Execute's report is accepted on its own terms: the trailing `”` is in the
stored reply (read from IndexedDB after a reload), and its reading of why
M11/M11b miss it is correct as specified — `stripUnmatchedLeadingQuote` fires
only on a leading quote, `stripSelfQuotingWrapper` requires a matched pair, and
this is a lone closer in sentence-final position. Three shapes, two rules.

**No fix is queued, for two reasons that compound:**

1. **It is n=1 on an engine that is no longer the default.** M15 was observed
   on WebLLM / Gemma 2 2B, as were M11, M11b and M14. As of `e5efb23` a
   first-time visitor boots on MediaPipe / Gemma 4 E2B, so every one of these
   artifacts moved from "the first thing a stranger sees" to "a thing an
   opt-in engine does". That does not make them fake; it makes them the wrong
   place to spend a gate read.
2. **The price is asymmetric.** `replyCleanup` is called from the App send
   path's finalize points, so any fix is gate-triggering. Under the README
   replay rule an `--rescore` satisfies it, which is cheap — but a
   third narrow string rule aimed at one observed reply, on an opt-in engine,
   is the kind of accumulation that makes cleanup code unreadable. M11b was
   worth it when the shape was on the default path. This one is not, yet.

**Re-open it if, and only if, a quote artifact of any shape appears on
MediaPipe.** That check costs nothing: R7's verification walk already drives
the new default two turns, so it records the answer as a by-product. If the new
default is clean, M15 stays closed and the M11/M11b rules stay as the record of
...
Measurement only, exactly as queued: **no `src/` diff** — this section plus two
screenshots is the whole deliverable. `npx vite preview` on `:4173` against a
production build (`npm run build` green), Chromium via Playwright, persistent
profile (same returning-user caveat as every audit walk since 07-21). Both
entries sent verbatim in both engines, one-variable. Engine switched in
**Settings → Privacy & your data → Inference Engine**; R1b's note held — the
`quietnote-runtime` localStorage key flipped immediately and the E2B model
loaded on the *next* boot, from an existing `transformers-cache` (privacy
dashboard read **6.2 GB** total browser storage before the switch), so no
3.15 GB download was needed. **0 console errors on every session.** The
runtime was set back to `webllm` at the end so the next run tests the shipped
default.

| # | engine | turn 2 vs turn 1 |
|---|---|---|
| 1 | WebLLM / Gemma 2 2B | **BYTE-IDENTICAL** |
| 2 | WebLLM / Gemma 2 2B | distinct |
| 3 | WebLLM / Gemma 2 2B | distinct |
| 4 | Transformers.js / Gemma 4 E2B | distinct |
| 5 | Transformers.js / Gemma 4 E2B | distinct |
| 6 | Transformers.js / Gemma 4 E2B | distinct |

**WebLLM 1/3, E2B 0/3.** Screenshots: `docs/screenshots/2026-08-02/m14a-webllm.png`
(session 3) and `m14a-transformersjs.png` (session 6); the repeat itself is
captured in `m11b-two-turn-freewrite.png` — session 1 was taken as M11b's
criterion (d) drive with the identical entries, so it counts as a WebLLM sample
rather than being re-run.

**This does not match any branch of the ruling's decision rule, and execute is
not inventing one.** The rule's three branches are E2B 0/3 **and WebLLM ≥2/3**;
E2B ≥1/3; WebLLM 0/3. The measured pair is E2B 0/3 **and WebLLM 1/3** — the
gap the rule did not anticipate. What the table does settle: the repeat is
**not** an every-session property of WebLLM (so the M14 filing's implicit "the
app is broken for everyone" framing is too strong), and it did **not** appear
on E2B in three tries (so nothing here contradicts model-specificity either).
Three sessions per engine cannot separate "1/3 vs 0/3" from noise. **For the
...
- Every E2B turn 2 was *substantively* turn-aware — it picked up "the moment I
  ...
  The distinctness is not just different wording.
- E2B sessions 1 and 2 both closed with an unprompted therapist suggestion
  ("Perhaps speaking with a therapist could offer some space…"), and both ran
  noticeably longer than the WebLLM replies. Not a defect and not in scope
...
**Raw pairs — PRUNED 2026-08-05 (planner).** All six were transcribed here in
full; M14b folded the rate into an n=10 table, M14 is now resolved by demotion
(the default is no longer WebLLM), so the five *distinct* pairs have no
remaining analytical use. Two things are kept because they are evidence rather
than illustration: the repeat itself, below, and the cross-session note that
follows it. The full pairs remain in PR #121's diff.

**Session 1 — WebLLM — REPEAT.** Both turns returned:
> Staying late at work and missing dinner with your friend must have felt disappointing. It sounds like the way it went down left you feeling a bit stuck in replaying that moment to yourself today, is there anything specific about what happened or why canceling made you feel so bad?

**One cross-session pattern worth the planner's eye, offered as observation not
ruling:** E2B's three turn-1 replies all open on the same "Missing dinner with
your friend…" construction, and sessions 4 and 6 share a clause verbatim
("sounds like leaving a gap where connection was supposed to be"). Cross-session
...
Ruled with no run in flight, on the artifacts execute filed plus the actual
code — not on the report's summary.

**The defect is CONFIRMED by reading the screenshot, not the ledger row.**
`docs/screenshots/2026-08-01/m11-verify-two-turn.png`: turn 1 and turn 2 render
the identical string — `Staying late at work can feel isolating.  What was the
moment you texted to cancel that felt so hard?` (note the doubled space, present
in both) — and turn 2 answers *precisely* that question ("It was the moment I
hit send…"). Execute's framing is right and worth keeping: this is a worse
stranger-facing defect than the quote artifacts, because the app does not look
clumsy, it looks broken. One correction to the filing: the quotes around the
reply in the queue item are execute quoting in prose — the rendered reply
carries no wrapper. M14 and M11/M11b are unrelated artifacts.

**Ruling on each of execute's three candidate shapes.**

| shape | ruling | why |
|---|---|---|
| (c) sampling changes | **REJECTED** — do not queue | `src/inference/webllm-engine.ts:78` already sends `repetition_penalty: options.repetitionPenalty ?? 1.3` (M0's engine-parity half, the surviving one). 1.3 is the *same* value that gave the headless E2B path near-zero echo in M1. The verbatim repeat happens at that setting. The knob is already at the value the evidence recommends, so turning it further is a guess, and it would be a send-path change carrying gate cost for a guess. |
| (a) drop WebLLM as the default engine | **NOT free, NOT ruled — priced below** | Mechanically it is two lines (`useInferenceEngine.ts:22`'s `return "webllm"` and `createEngine`'s `runtime: RuntimeId = "webllm"` default at `inference/index.ts:12`), and it is *strictly less* than the removal Sharang gated on 2026-07-12 — WebLLM would stay selectable in Settings. But R1b measured the swap's real cost: **the first-run download goes 1.49 GB → 3.15 GB**, more than double, for every stranger. public-release's own P0 is a stranger reaching one successful exchange; doubling the download is the most direct way to make that harder. It also invalidates R2b's honest size line, R3a's README sizes, and the R1b matrix in one move. This is a trade, not a fix, and it is Sharang's trade. |
| (b) repeat-detector that re-prompts | **PLAUSIBLE, deferred to M14a's result** | The precedent is good: `responseShaping.ts` (`isBareDeflection` → `withDeflectionReprompt`) and `referralReprompt.ts` are both exactly this shape, and a detector is engine-agnostic. Two caveats that must be settled before it is queued, not after: (i) it papers over a model this initiative has *already* recommended removing, and (ii) it changes the send path's message construction, so under the README's replay rule it is gate-triggering **in the expensive way** unless it is *proved* — not assumed — that it cannot fire on the suite (the eval cases are single-turn, so a detector needing ≥2 assistant turns should never fire; `referralReprompt` does fire ~3×/mode, so "re-prompts never fire in eval" is demonstrably not a safe blanket assumption). |

**The missing variable.** Every shape above is chosen differently depending on
one unmeasured fact: **does the Transformers.js / Gemma 4 E2B path repeat too?**
Nobody has driven it two turns. If it does not repeat, (a) becomes a genuine fix
with a known price and the decision goes to Sharang with numbers attached. If it
does repeat, (a) buys 1.66 GB of extra download for nothing and (b) is the only
shape left. That is a ~one-evening measurement deciding a change worth hours of
gate time — so it goes first. Queued as **M14a**.

**Decision rule for the next planning run** (fixed now, so the answer is not
reverse-engineered from whatever the table shows):
- E2B repeats in **0 of 3** sessions and WebLLM in **≥2 of 3** → the defect is
  model-specific; take the default-swap question to Sharang with the 1.49→3.15 GB
  price stated plainly, and do **not** queue a detector.
- E2B repeats in **≥1 of 3** → the defect is not model-specific; queue the (b)
  detector against `responseShaping.ts`'s existing shape, with the never-fires-in-eval
  claim carried as a *test assertion*, not a note.
- WebLLM repeats in **0 of 3** → M14's own evidence was a one-off after all;
  downgrade it to a WATCH line and queue nothing. (M1b's turn-5 loops still stand
  as separate evidence and stay on the record either way.)

**Standing note, unchanged by any of the above:** the fine-tune targets Gemma 4
E2B. WebLLM / Gemma 2 2B will never receive it. Every M2–M13 dollar and hour has
been spent improving a model that is **not** what a first-time stranger currently
loads. That mismatch is the real subject of M14, and it is Sharang's to resolve
(see Blocked on Sharang).
...
|---|---|---|---|
| 2026-08-05 | M5b / R7 — the default engine is now Gemma 4 E2B (MediaPipe) | #125 | Cross-listed from `public-release.md` (R7) because it changes **which model answers a stranger** — the model this initiative fine-tunes. Landed Sharang's `tasks-genai` 0.10.27→0.10.29 bump and the default swap, plus a test pinning both halves of the default. **Consequences for this initiative, stated plainly:** (a) M11/M11b/M15's quote artifacts and M14's verbatim repeat are all **WebLLM** observations — the default no longer runs WebLLM, so none of them is on a stranger's path any more, though all three remain real on a selectable engine; (b) the price is echo risk — MediaPipe has no repetition-penalty knob and M1b measured it 7/10 no-echo (mean overlap 0.22, one hard fail 0.84 on `echo-fw-3`); (c) **the release-gate floors have never been read on this model** — that read is **M16**, and it is a soft-launch blocker, not a merge blocker. **One free sample added this run** (R7 step 4, same two entries as M14a/b/c): MediaPipe turn 2 **distinct**, no repeat, **no quote artifact of any kind** → MediaPipe now **0 of 4** on the M14 repeat metric, and the **M15 re-open condition did not fire**. |
...
- **WebLLM removal — go/no-go** (added 2026-07-16): the M1b data is in and
  ...
  **Sharpened 2026-08-02 (M14 ruling) — the question now has a price tag and
  fresh evidence.** New evidence: the shipped default returned a
  **byte-identical reply on turn 2** in the M11 verification session
  (screenshot committed) — not turn 5, turn 2, i.e. a stranger's second message.
  New price: R1b measured the default-swap's cost as **1.49 GB → 3.15 GB of
  first-run download**, more than double, against public-release's own P0 of a
  stranger reaching one successful exchange. So the go/no-go is no longer
  "remove a worse model", it is **"is a coherent second turn worth doubling the
  download?"** — and the loop will not decide that for you. M14a (queued, free)
