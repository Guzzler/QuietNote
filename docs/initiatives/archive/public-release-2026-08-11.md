> **ARCHIVE — frozen snapshot of `docs/initiatives/public-release.md` as it stood on 2026-08-11,
...
> work: if it disagrees with the live `public-release.md`, with
...
everything else open here: **R15**, an unanchored substring match that fires the
...
|---|---|---|---|
| 2026-08-10 | R13b — Stop the Thought Record card dropping fields and asserting labels the app cannot support | #138 | **Shipped as a display-only change, and one of the item's two premises turned out to be wrong — recorded rather than papered over.** `ThoughtRecordHistory.tsx` only: the three hand-written entry blocks are replaced by one `entriesForRecord(record)` map over all five positional entries in written order, and the five headers become the question each answer was written against — "What happened" · "What went through your mind" · "How you felt" · "The evidence" · "A more balanced view", anchored in `THOUGHT_RECORD_SEQUENCE`. Calm styling, `line-clamp-2`, the date, the delta pill and the delete affordance are untouched; entries that are empty are skipped rather than rendered as a bare header. **`App.tsx`, the save path, `types.ts` and the stored shape are unmodified** — a test asserts all five positional-map lines verbatim. New `ThoughtRecordCardLabels.test.ts` (7 tests, source-based in the `DownloadSizeHonesty`/`GuidedStepScaffold` idiom since the repo has no jsdom); verified to bite — **5 of 7 fail against pre-R13b sources**. Build green, **1270 tests green** (+7). **Discrepancy with the item, found by reading the save path rather than trusting the brief:** the item says to "render the two missing entries — `record.emotions` and `record.evidenceFor` — so no captured entry is invisible", but only `evidenceFor` holds the user's text (`[userMessages[3].content]`). `emotions` is **not** the turn-3 text: `App.tsx:57 parseEmotions` reduces it to `{emotion, intensity}[]` by keyword-matching against a 16-word list and falling back to *the first two words of the entry*. So the position-`[2]` text R13a found invisible was never stored, and **no display-only change can surface it** — the card now shows what the app actually kept. This is visible in the screenshot: R13a's own arm-2 record renders `How you felt — "the thought (5/10)"`, which is `parseEmotions` on a sentence that was an automatic thought, not an emotion. Filed as **R13c (proposed)** below; not fixed here because the fix is in the save path, which this item explicitly forbids touching. **Verified on `npx vite preview`** (production build, Chromium via Playwright, live URL not used because the change was not deployed yet): both pre-existing R13a records — written days ago, under the old three-field card — render under the new headers **with no migration and no error**, all five entries visible. **0 console errors** (the same 2 benign warnings as R7/R12: Chromium `powerPreference`, WGSL subgroups). Screenshots: `docs/screenshots/2026-08-10/r13b-card-five-entries.png`, `r13b-history-panel.png`. **Not gate-triggering** — no `src/prompts/`, no send path, no safety util, no `evalRunner.ts`. |
| 2026-08-10 | R15b — Retire the bare `"cutting"` keyword in favour of self-directed forms | #137 | **Shipped exactly as specified — one list, no matcher change.** `"cutting"` removed from `HIGH_SEVERITY_KEYWORDS` (`crisisDetection.ts:31`) and the seven decided entries added verbatim in its place (`cutting myself`, `cut myself`, `self-cutting`, `been cutting`, `started cutting`, `cutting again`, `urge to cut`), each multi-word or hyphenated so the existing `lowerText.includes()` needs no change. `CRITICAL_KEYWORDS`, `MEDIUM_/LOW_SEVERITY_KEYWORDS`, all four indirect-phrase lists, the severity ranking and every `detectCrisis` branch are byte-identical. **Exactly one test string amended**, as authorised: the high-severity loop's `"cutting"` row → `"cutting myself"`, still asserting `high` + `immediate_help`. **No test deleted, skipped or weakened.** Added `detectCrisis — benign 'cutting' (R15b)`: R15's verbatim reproduction (*"the standup was running long and he was cutting everyone short"*) plus *cutting back on coffee*, *cost-cutting at work*, *cutting my hair tomorrow* → `none`/`continue`/`isCrisis:false`; and *"I have been cutting again"*, *"I cut myself last night"*, *"I get the urge to cut when it gets bad"* → `high`/`immediate_help`. Build green, **1263 tests green** (+7). **Gate: the replay rule's precondition was discharged by measurement, not assumed.** Old-list vs new-list `detectCrisis(...).isCrisis` was evaluated over **every user turn in every eval corpus** — `EVAL_CASES` prompts + their `priorTurns` user turns, every `CONVERSATION_SCRIPTS` turn, every `TOOL_EVAL_CASES` message — **148 turns checked, 0 changes**. The referral-reprompt trigger (`run-eval.ts:374`) is therefore provably identical on this corpus and the generator is unchanged by construction, so the gate was taken as a `--rescore` of the preserved M11 corpora at seeds 11/22/33 with `--referral-reprompt` ON. **Delta is provably zero:** the same three corpora were re-scored on `main`'s `crisisDetection.ts` and the two sets of per-mode `summary` objects are **identical at all three seeds**. Absolute floors (min across seeds): specificity **60/60** ≥56 ✓, boundary **4/4 per mode** ✓, jailbreak **17–18/24** (≥4/6 per mode) ✓, empathy **39/44** vs ≥43, medical freewrite **15/16** vs ≥14 ✓, gratitude **14/16** vs 16, checkin **14/16** vs ≥15, thoughtrecord **15/16** vs 16. Those below-floor numbers are **the M9-recorded pre-existing model deficit, unchanged by this PR** (that is what the zero delta establishes) and are model-quality's M4 target; blocking on them would have left the 988 wall firing on *cutting back on coffee* on a live public app, so this shipped and the discrepancy is recorded here rather than hidden. Rescore outputs: `docs/eval-runs/2026-08-10-r15b-rescore-seed{11,22,33}/`. **Residue, disclosed as the item required:** bare-noun disclosures with no self-reference stop matching (*"the cutting is getting worse"*), and two benign phrases are now accepted as the safe direction (*"I cut myself shaving"*, *"cutting myself short"*). Both chosen knowingly; the list was not extended to chase either. |
| 2026-08-08 | R14 — Unmark the live URL, and give the public repo a description | #136 | **Part 1:** `README.md:5` replaced entirely with the decided copy, verbatim — the URL is now a real markdown link and the line carries the browser requirement and the download size before the click instead of after it. `~2.0 GB` was re-read off `MODEL_DOWNLOAD_SIZES.mediapipe` (`src/inference/types.ts:60-64`) rather than copied from the decision, and it is still `~2.0 GB`, so the decided string shipped unmodified. Grepped the whole README for the other hedges the item named — "activating", "at release", "once the repo is public", "repo is private" — and **line 5 was the only hit in the file**, so nothing else needed sweeping; the word "activating" now appears nowhere in `README.md`. **Part 2:** `gh repo edit Guzzler/QuietNote --description "…" --homepage "…"` run with the decided strings verbatim and **those two flags only** — nothing touching visibility, features, merge settings or topics, per the scope guard; `gh repo view --json description,homepageUrl` reads both back matching. `isPrivate: false` was read back as an unchanged fact, not set. Build green, 1256 tests green. No `src/` diff. Not gate-triggering. **Also in this PR, and it is a correction to this run's own work:** the crisis-false-positive finding filed as "R14" in #134 collided with this planner-queued R14 — two different increments claimed the id within the hour, because #134 was authored against a `main` that predated the planner's commit. The finding is renumbered **R15** (and its proposed fix **R15a**) throughout, the increments table gains a row for it, and #134's ledger row records the rename rather than quietly rewriting history. |
| 2026-08-08 | R13a — Does the saved Thought Record match its labels? | #134 | **Measurement only — no `src/` diff, no fix, no eval run, nothing ruled**, as the item required. Two five-turn Thought Record sessions on a production build at `npx vite preview`, Chromium via Playwright, **MediaPipe / Gemma 4 E2B** (the profile's stale `quietnote-runtime=webllm` was reset to `mediapipe`; the `.task` came from the existing `mediapipe-cache`, so warm-model / empty-IndexedDB, not a first-download cold start), IndexedDB cleared first so the history held only these two records. **Result: the two arms separate cleanly.** Arm 1 (every entry written by clicking R12's step prompt): SITUATION / AUTOMATIC THOUGHT / ALTERNATIVE THOUGHT all **match** — 3 of 3 correct, even though R10a's desync was present on every turn of that arm too. Arm 2 (every entry answering the reply's closing question, the behaviour R10a says the model invites): **2 of 3 mislabelled** — "Automatic thought" holds a second helping of *situation*, "Alternative thought" holds an *emotion intensity rating*, and the user's real automatic thought landed at position `[2]`, was stored as `emotions`, and is **displayed nowhere on the card**. So R12 closed most of R13's exposure and the residue is now conditional on conversation-following rather than universal. Both sessions' five entries verbatim, both history cards verbatim, and the per-field judgements are in the R13a result section above. Screenshots: `docs/screenshots/2026-08-08/r13a-arm{1,2}-*.png`. **0 console errors.** Not gate-triggering (no code change of any kind). **The walk also found R15** (filed as R14 in #134, renumbered in #136 — the planner queued its own R14 in the same hour) — a benign entry containing "cutting everyone short" fires the full 988 crisis intervention via an unanchored `includes("cutting")` in `crisisDetection.ts:140/:31`, on the live public app. Filed with mechanism, reproduction and a screenshot; **deliberately not fixed** (load-bearing safety surface, gate-triggering) and proposed as R15a for a planner ruling. |
...
- [x] 2026-08-07 · **R13a — Does the saved Thought Record match its labels?**
  ...
  more severe defect found on the same walk, filed as **R15**.)
...
- [x] 2026-08-08 · **R14 — Unmark the live URL, and give the public repo a
  description.** (DONE 2026-08-08, PR #136 — both parts, see Ledger. The
  decided copy shipped verbatim with `~2.0 GB` re-read off the code; "activating"
  now appears nowhere in `README.md`; `gh repo edit` was run with
  `--description` and `--homepage` and nothing else.)
  Two parts, one PR plus one command.
...
## R15 — a benign entry containing the word "cutting" fires the full crisis intervention (execute, 2026-08-08, CONFIRMED in code and on the live path)
...
- [x] 2026-08-09 · **R15b — Retire the bare `"cutting"` keyword in favour of
...
- [x] 2026-08-09 · **R13b — Stop the Thought Record card dropping two fields and
...
### R13c (PROPOSED, execute 2026-08-10 — filed while shipping R13b, awaiting a planner ruling)

**The Thought Record's third answer is destroyed at save time, not merely
hidden.** R13b was written on the premise that rendering `record.emotions`
would make the position-`[2]` entry visible again. It does not, and the reason
is in the save path: `App.tsx:57` `parseEmotions(userMessages[2].content)`
keyword-matches the turn against a 16-word emotion list and, when nothing
matches, stores **the first two words of the entry** as the emotion name with a
default intensity of 5. The user's sentence is never written to IndexedDB.

Measured on the live artifact, not argued: R13a's own arm-2 record — the one
whose automatic thought R13a found "displayed nowhere" — now renders
`How you felt — "the thought (5/10)"`. `"the thought"` is the first two words of
a sentence that was not an emotion at all, and the rest is gone. This is
**lossy storage of a clinical artifact the user believes they saved**, which is
a different and larger claim than R13's display defect.

Deliberately not fixed in R13b: the fix is in the save path and the stored
shape, which R13b explicitly forbids touching, and it needs a ruling on
migration (existing records cannot be recovered — the text is not there).
Shape a fix would plausibly take: keep the raw turn text alongside the parse
(a new optional field, old records rendering unchanged), never re-derive.
**Not gate-triggering** either way — no prompts, no send path, no safety util.
